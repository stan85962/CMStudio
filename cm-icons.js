// ===== ICÔNES SVG — Style Lucide (strokeWidth 1.5, viewBox 0 0 24 24) =====
// Usage : icon('bell', 16)  →  string SVG inline
...
const ICONS = {
  ...
  // Statut / Connexion
  wifiOff:      '<path d="M12 20h.01"/><path d="M8.5 16.429a5 5 0 0 1 7 0"/><path d="M5 12.859a10 10 0 0 1 5.17-2.69"/><path d="M19 12.859a10 10 0 0 0-2.007-1.523"/><path d="M2 8.82a15 15 0 0 1 4.177-2.643"/><path d="M22 8.82a15 15 0 0 0-11.288-3.764"/><line x1="2" x2="22" y1="2" y2="22"/>',
  keyRound:     '<path d="M2 18v3c0 .6.4 1 1 1h4v-3h3v-3h2l1.4-1.4a6.5 6.5 0 1 0-4-4Z"/><circle cx="16.5" cy="7.5" r=".5" fill="currentColor"/>',
  dna:          '<path d="M2 15c6.667-6 13.333 0 20-6"/><path d="M9 22c1.798-1.998 2.518-3.995 2.807-5.993"/><path d="M10 9c-.747-1.997-1.466-3.995-2.807-5.993"/><path d="m17 2-2.5 2.5"/><path d="m19 4-7.5 7.5"/><path d="m21 6-1.5 1.5"/><path d="m3 14 1.5-1.5"/><path d="m5 16 7.5-7.5"/><path d="m7 18 2.5-2.5"/><path d="M2 9c6.667 6 13.333 0 20 6"/>',
};

/**
 * Retourne un SVG inline Lucide pour le nom donné.
 * @param {string} name   Clé dans ICONS
 * @param {number} size   Taille px (défaut 16)
 * @returns {string}      SVG string ou '' si introuvable
 */
function icon(name, size = 16) {
  const body = ICONS[name];
  if (!body) return '';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">${body}</svg>`;
}
